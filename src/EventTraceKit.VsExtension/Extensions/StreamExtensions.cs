namespace EventTraceKit.VsExtension.Extensions
{
    ...
    public static class StreamExtensions
    {
        ...
        /// <summary>
        ///   Reads a 4-byte unsigned integer (<see cref="uint"/>) from the specified
        ///   stream.
        /// </summary>
        /// <param name="input">The stream from which to read from.</param>
        /// <returns>The integer read from the input.</returns>
        /// <exception cref="ArgumentNullException">
        ...
        public static uint ReadUInt32(this Stream input)
        {
            ...
            ReadExact(input, buffer, 0, buffer.Length);
            return BitConverter.ToUInt32(buffer, 0);
        }

        /// <summary>
        ///   Asynchronously reads a 4-byte unsigned integer (<see cref="uint"/>)
        ...
        public static async Task<uint> ReadUInt32Async(
            this Stream input, CancellationToken cancellationToken)
        {
            var buffer = new byte[sizeof(uint)];
            await ReadExactAsync(input, buffer, 0, buffer.Length, cancellationToken);
            return BitConverter.ToUInt32(buffer, 0);
        }
    }
}
