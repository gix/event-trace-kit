namespace InstrManifestCompiler.Support
{
    ...
    internal unsafe sealed class MD5Managed : HashAlgorithm
    {
        ...
        public void Initialize(byte[] state)
        {
            Initialize();
            fixed (MD5_CTX* ctx = &context) {
                ctx->state[0] = BitConverter.ToUInt32(state, 0);
                ctx->state[1] = BitConverter.ToUInt32(state, 4);
                ctx->state[2] = BitConverter.ToUInt32(state, 8);
                ctx->state[3] = BitConverter.ToUInt32(state, 12);
            }
        }
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            ...
            fixed (byte* ptr = array)
                MD5Update(ctx, ptr + ibStart, (uint)cbSize);
        }
        ...
        class Buffer
        {
            public static void Clear(void* ptr, int index, int length)
            {
                var p = (byte*)ptr;
            ...
            public static void BlockCopy(void* input, void* output, uint length)
            {
                var src = (byte*)input;
                var dst = (byte*)output;
        ...
        {
            public fixed uint state[4];   /* state (ABCD) */
            public fixed uint count[2];   /* number of bits, modulo 2^64 (lsb first) */
            public fixed byte buffer[64];  /* input buffer */

            public void Clear()
            {
                fixed (MD5_CTX* s = &this) {
                    Buffer.Clear(s->state, 0, sizeof(uint) * 4);
                    Buffer.Clear(s->count, 0, sizeof(uint) * 2);
                    Buffer.Clear(s->buffer, 0, sizeof(byte) * 64);
                }
            }
        }
